...
from common import (
    apply_style, classify_and_answer, family_management_ui, format_phone,
    install_clear_cache_shortcut_guard, install_phone_input_guard,
    load_logs_for_user, render_landing_preview, save_usage_log,
    show_auth, show_error, show_log_summary,
)
...
def show_parent_main():
    install_phone_input_guard(); install_clear_cache_shortcut_guard()
    user = st.session_state.eumi_user
    apply_style("이음이 부모님용", f"{user.get('name','사용자')}님, 어려운 화면은 사진으로 물어보세요", theme="parent", badge="부모님용")
    render_landing_preview("parent")
    col_a, col_b = st.columns([3, 1])
    with col_a:
        st.markdown(f"<div class='small-note'><b>내 번호:</b> {format_phone(user.get('phone',''))}</div>", unsafe_allow_html=True)
    with col_b:
        if st.button("로그아웃"):
            logout()
    ...
    with tab_help:
        ...
        st.markdown("""
<div class='guide-card'>키오스크, 병원 앱, 은행 앱, 기차 예매 화면 등<br><b>막히는 화면을 사진으로 찍어서 올려주세요.</b><br>이전에 비슷한 질문이 있으면 같이 참고해서 알려드립니다.</div>
""", unsafe_allow_html=True)
        uploaded_file = st.file_uploader("사진 선택", type=["jpg", "jpeg", "png"], label_visibility="collapsed")
        question = st.text_input("질문", value="이 화면에서 다음에 뭘 누르면 되나요?", key="help_question")
        if uploaded_file:
            ...
            if st.button("✨ AI에게 물어보기"):
                with st.spinner("AI가 화면을 분석하고 있어요..."):
                    try:
                        ...
                        st.markdown(f"""
<div class='folder-card'><b>자동 분류</b><br>📁 {result.get('category')} &gt; {result.get('place_name')}<br>하려는 일: {result.get('task_name')}</div>
""", unsafe_allow_html=True)
...
if st.session_state.eumi_user is None:
    show_auth("eumi_user", title="이음이 부모님용", subtitle="어려운 화면, 사진 찍어서 물어보세요", theme="parent", badge="부모님용")
else:
    show_parent_main()
