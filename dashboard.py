...
from common import (
    apply_style, family_management_ui, format_phone, get_user_name,
    install_clear_cache_shortcut_guard, install_phone_input_guard,
    load_logs_for_user, load_parents_i_help, render_landing_preview,
    show_auth, show_log_summary,
)
...
def show_child_dashboard():
    ...
    apply_style("이음이 자녀 대시보드", f"{user.get('name','사용자')}님, 부모님의 디지털 사용 기록을 확인하세요", theme="child", badge="자녀용")
    ...
    with tab_parents:
        ...
        if not links:
            st.markdown("""
<div class='guide-card'>아직 연결된 부모님이 없습니다.<br><b>가족 관리</b> 탭에서 부모님 전화번호를 입력해 연결 요청을 보내세요.</div>
...
if st.session_state.dashboard_user is None:
    show_auth("dashboard_user", title="이음이 자녀 대시보드", subtitle="부모님의 디지털 사용 기록을 함께 확인하세요", theme="child", badge="자녀용")
else:
